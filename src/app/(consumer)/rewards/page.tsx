...
// Mock reward data
const mockReward = {
  current_tier: 'explorer' as const,
  total_stars: 1,
  current_streak_weeks: 0,
  longest_streak_weeks: 1,
  unique_venues_visited: 1,
  next_tier_stars: 3,
}

export default function RewardsPage() {
  const currentTierIndex = tiers.findIndex((t) => t.key === mockReward.current_tier)
  const progressPct = Math.min(100, (mockReward.total_stars / mockReward.next_tier_stars) * 100)
    ...
    <div className="min-h-screen px-4 md:px-8 py-8 max-w-lg mx-auto space-y-6">
      ...
      <div className="p-6 rounded-2xl text-center space-y-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">{mockReward.total_stars} Sterne</p>
        </div>
        ...
        {currentTierIndex < tiers.length - 1 && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-[var(--text-tertiary)]">
              Noch {mockReward.next_tier_stars - mockReward.total_stars} Buchungen bis {tiers[currentTierIndex + 1].label}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="p-5 rounded-2xl space-y-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-[var(--text-primary)]">{mockReward.current_streak_weeks} Wochen</p>
          </div>
          <div className="text-3xl" style={{ color: mockReward.current_streak_weeks > 0 ? '#FF9F0A' : 'var(--text-tertiary)' }}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill={mockReward.current_streak_weeks > 0 ? '#FF9F0A' : 'none'} stroke={mockReward.current_streak_weeks > 0 ? '#FF9F0A' : 'var(--text-tertiary)'} strokeWidth="1.5"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z" /></svg>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-2xl font-bold text-[var(--text-primary)]">{mockReward.unique_venues_visited}</p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">Venues besucht</p>
        </div>
        <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-2xl font-bold text-[var(--text-primary)]">{mockReward.longest_streak_weeks}</p>
