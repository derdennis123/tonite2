  ...
  if (sent) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
        ...
        <button type="button" onClick={() => setSent(false)} className="mt-8 text-sm text-[var(--accent-primary)]">Andere E-Mail verwenden</button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm space-y-8">
        ...
        {error && (
          <div className="px-4 py-3 rounded-xl text-sm text-[var(--danger)]" style={{ background: 'rgba(255,69,58,0.1)', border: '1px solid rgba(255,69,58,0.2)' }}>
            {error}
          </div>
        )}

        {/* Google Login */}
        <button type="button" onClick={handleGoogleLogin} className="w-full py-3.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-3 transition-all hover:bg-[var(--bg-glass-hover)]" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-primary)' }}>
